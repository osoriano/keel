...
class NotificationEventListener(
  ...
  fun onUnpinnedNotification(notification: UnpinnedNotification) {
    with(notification) {
      ...
      val latestArtifact = repository.getArtifactVersion(pinnedEnv.artifact, latestApprovedArtifactVersion)
      val pinnedArtifact = repository.getArtifactVersion(pinnedEnv.artifact, pinnedEnv.version)
  ...
  fun onPluginNotification(notification: PluginNotification) {
    with(notification) {
      ...
      val artifact = repository.getArtifactVersion(
        deliveryArtifact, constraintStateChanged.currentState.artifactVersion
      ) ?: run {
  ...
  fun onArtifactVersionDeployFailed(notification: ResourceTaskFailed) {
    ...
    val latestArtifact = repository.getArtifactVersion(artifact, latestApprovedVersion) ?: run {
      ...
      if (shouldSendManualJudgementAwait() || shouldSendManualJudgementUpdate()) {
        ...
        val pinnedArtifact =
          repository.getPinnedVersion(config, currentState.environmentName, deliveryArtifact.reference)?.let {
            repository.getArtifactVersion(deliveryArtifact, it)
  ...
  private fun getConfigAndArtifact(application: String? = null,
    ...
    val artifact = repository.getArtifactVersion(deliveryArtifact, version) ?: run {
            ...
            "✅ &nbsp; Deployed `$version` to [${resource.name}]($environmentsLink)\n" +
              endpoints.map { (region, regionEndpoints) ->
                ...
                regionEndpoints.joinToString { endpoint ->
                  // TODO: determine the right port for these links
                  "[${endpoint.type.friendlyName}](${endpoint.protocol}://${endpoint.address})"
                }
            }.joinToString("\n")
          } else {
