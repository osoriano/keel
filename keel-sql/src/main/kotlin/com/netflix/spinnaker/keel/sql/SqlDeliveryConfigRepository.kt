...
class SqlDeliveryConfigRepository(
  ...
  override fun getMigratableApplicationData(application: String): ApplicationPrData {
    return sqlRetry.withRetry(READ) {
      jooq
        .select(MIGRATION_STATUS.AUTO_GENERATED_CONFIG,
  ...
  override fun storeUserGeneratedConfigForMigratedApplication(application: String, deliveryConfig: SubmittedDeliveryConfig) {
    sqlRetry.withRetry(WRITE) {
      jooq.update(MIGRATION_STATUS)
      ...
      when {
        exists && !migrating -> ApplicationMigrationStatus(alreadyManaged = true)
          ...
          jooq
            ...
            .from(MIGRATION_STATUS)
            .where(MIGRATION_STATUS.APPLICATION.eq(application))
            .fetchOne { (exportSucceeded, inAllowList, assistanceNeeded, deliveryConfig, prLink, jiraLink, pipelines) ->
