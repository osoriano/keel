...
class SqlDeliveryConfigRepository(
  ...
  override fun storeAppForPotentialMigration(app: String, inAllowedList: Boolean?): Boolean {
    return sqlRetry.withRetry(WRITE) {
      jooq.insertInto(MIGRATION_STATUS)
        .set(MIGRATION_STATUS.APPLICATION, app)
        .set(MIGRATION_STATUS.IN_ALLOW_LIST, inAllowedList ?: false)
        .apply {
          if (inAllowedList != null) {
            onDuplicateKeyUpdate()
              .set(MIGRATION_STATUS.IN_ALLOW_LIST, inAllowedList)
          } else {
            onDuplicateKeyIgnore()
          }
        }
  ...
  override fun getMigratableApplicationData(application: String): ApplicationPrData {
    return sqlRetry.withRetry(READ) {
      jooq
        .select(
          MIGRATION_STATUS.AUTO_GENERATED_CONFIG,
          MIGRATION_STATUS.USER_GENERATED_CONFIG,
          MIGRATION_STATUS.REPO_SLUG,
          MIGRATION_STATUS.PROJECT_KEY,
          MIGRATION_STATUS.PR_LINK
        )
        .from(MIGRATION_STATUS)
        .where(MIGRATION_STATUS.APPLICATION.eq(application))
        .fetchOne { (autoGeneratedConfig, userGeneratedConfig, repoSlug, projectKey, prLink) ->
          ApplicationPrData(
            autoGeneratedConfig = objectMapper.readValue(autoGeneratedConfig),
            userGeneratedConfig = userGeneratedConfig?.let { objectMapper.readValue(it) },
            repoSlug = repoSlug,
            projectKey = projectKey,
            prLink = prLink
          )
        } ?: throw ApplicationPullRequestDataIsMissing(application)
    }
  }
  ...
  override fun storeUserGeneratedConfigForMigratedApplication(
    ...
    sqlRetry.withRetry(WRITE) {
      jooq.update(MIGRATION_STATUS)
        .set(MIGRATION_STATUS.USER_GENERATED_CONFIG, deliveryConfig.toJson())
        .set(MIGRATION_STATUS.USER_GENERATED_AT, clock.instant())
        .set(MIGRATION_STATUS.USER_GENERATED_BY, user)
        .where(MIGRATION_STATUS.APPLICATION.eq(application))
        .execute()
    }
  }
  ...
  override fun getApplicationMigrationStatus(application: String): ApplicationMigrationStatus {
    return sqlRetry.withRetry(READ) {
      val (exists, migrating) = jooq
        ...
        .where(DELIVERY_CONFIG.APPLICATION.eq(application))
        .fetchOne { (metadata) ->
          Pair(true, metadata?.get(MIGRATING_KEY) as? Boolean ?: false)
        }
        ?: Pair(false, false)
        ...
        exists && !migrating -> ApplicationMigrationStatus(alreadyManaged = true)
          ...
          jooq
            .select(
              ...
              MIGRATION_STATUS.USER_GENERATED_CONFIG,
              MIGRATION_STATUS.USER_GENERATED_AT,
              MIGRATION_STATUS.USER_GENERATED_BY
            )
            .from(MIGRATION_STATUS)
            .where(MIGRATION_STATUS.APPLICATION.eq(application))
            .fetchOne { (exportSucceeded, inAllowList, assistanceNeeded, deliveryConfig, prLink, jiraLink, pipelines, userGeneratedConfig, userGeneratedAt, userGeneratedBy) ->
              ApplicationMigrationStatus(
                ...
                autoGeneratedConfig = deliveryConfig?.let { objectMapper.readValue(deliveryConfig) },
                prLink = prLink,
                jiraLink = jiraLink,
                pipelines = pipelines?.let { objectMapper.readValue(pipelines) },
                userGeneratedConfig = userGeneratedConfig?.let {
                  UserGeneratedConfig(
                    config = objectMapper.readValue(it),
                    updatedAt = userGeneratedAt,
                    updatedBy = userGeneratedBy
                  )
                }
              )
            }
  ...
  override fun storePipelinesExportResult(
    ...
    sqlRetry.withRetry(WRITE) {
      jooq.update(MIGRATION_STATUS)
        ...
        .set(MIGRATION_STATUS.AUTO_GENERATED_CONFIG, deliveryConfig.toJson())
