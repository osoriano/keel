...
import com.netflix.spinnaker.keel.exceptions.ActiveDeployments
...
internal class EnvironmentExclusionEnforcerTest {
  ...
  private val enforcer =
    EnvironmentExclusionEnforcer(springEnv, verificationRepository, artifactRepository, environmentLeaseRepository)
  private val verification = DummyVerification("1")
  private val environment = com.netflix.spinnaker.keel.api.Environment(name = "test", verifyWith = listOf(verification))
  ...
  fun `withVerificationLease invokes the action when there are no pending verifications or deployments`() {
    ...
    every { verificationRepository.getVerificationContextsWithStatus(any(), any(), PENDING) } returns emptyList()
  ...
  fun `withActuationLease does not invoke the action when there are pending verifications`() {
    every { verificationRepository.getVerificationContextsWithStatus(any(), any(), PENDING) } returns
  ...
  fun `withVerificationLease throws an exception when there are pending verifications`() {
    every { artifactRepository.isDeployingTo(any(), any()) } returns false
    every { verificationRepository.getVerificationContextsWithStatus(any(), any(), PENDING) } returns listOf(
      mockk(relaxed = true)
    )
  ...
  fun `withVerificationLease  throw an exception when there is an active lease`() {
    val environment = mockk<com.netflix.spinnaker.keel.api.Environment> {
      every { name } returns "testing"
    }
    every { environmentLeaseRepository.tryAcquireLease(any(), any(), any()) } throws ActiveLeaseExists(
      environment, "mystery", mockk()
    )
    val action : () -> Unit = mockk(relaxed = true)
    expectCatching { enforcer.withVerificationLease(mockk(relaxed = true), action) }
      .isFailure()
  ...
  fun `withVerificationLease invokes the action whether the feature flag is enabled or not`(enabled: Boolean) {
    every {
      springEnv.getProperty(
        "keel.enforcement.environment-exclusion.enabled", Boolean::class.java, any()
      )
    } returns enabled
