...
import com.netflix.spinnaker.keel.artifacts.ArtifactDeployingListener.Companion.ARTIFACT_DEPLOYMENT_DELAY
...
internal class ArtifactDeployingListenerTests {
  ...
  fun setup() {
    ...
    every { repository.getPinnedVersion(config, any(), any()) } returns null
  ...
  fun `does nothing if artifact version is not approved for env`() {
    every { repository.getApprovedAt(any(), any(), event.artifactVersion, any()) } returns null
    subject.onArtifactVersionDeploying(event)
    verify(exactly = 0) { repository.markAsDeployingTo(config, any(), event.artifactVersion, any()) }
  }
  ...
  fun `records deployment delay when version is pinned`() {
    every { repository.getPinnedVersion(config, any(), any()) } returns event.artifactVersion
    subject.onArtifactVersionDeploying(event)
    verifyDeploymentDelayRecorded("pinned")
  }

  private fun verifyDeploymentDelayRecorded(action: String) {
    val tags = slot<Set<Tag>>()

    verify { spectator.timer(ARTIFACT_DEPLOYMENT_DELAY, capture(tags)) }
    verify { timer.record(any<Duration>()) }

    expectThat(tags.captured).one {
