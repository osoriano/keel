...
final class WorkQueueProcessor(
  ...
  private fun PublishedArtifact.completeDockerImageDetails(): PublishedArtifact {
    ...
    return copy(
      ...
      gitMetadata = GitMetadata(buildDetail.commitId),
      buildMetadata = BuildMetadata(buildDetail.buildNumber, uid = buildDetail.buildId),
      metadata = metadata + mapOf("createdAt" to Instant.ofEpochMilli(buildDetail.completedAt))
    )
  }
...
internal val PublishedArtifact.buildController: String?
  get() = metadata["controllerName"] as? String

internal val PublishedArtifact.buildJob: String?
  get() = metadata["jobName"] as? String

internal val PublishedArtifact.isIncompleteDockerArtifact: Boolean
  get() = type == DOCKER && name.contains(".properties")
