...
abstract class ActionRepositoryTests<IMPLEMENTATION : ActionRepository> {
  ...
  @Test
  fun `multiple contexts, multiple delivery configs, multiple verifications`() {
    val v1 = verification
    val v2 = DummyVerification("2")

    val e1 = environment
    val e2 = Environment(
      // Same name as other environment
      name = e1.name,
      verifyWith = listOf(v2)
    )

    val d1 = deliveryConfig
    val d2 = DeliveryConfig(
      application = "fnord2",
      name = "fnord-manifest2",
      serviceAccount = "jamm2@illuminati.org",
      environments = setOf(e2),
      // Same artifacts as other delivery config, but with a different deliveryConfigName
      artifacts = setOf(
        DockerArtifact(
          name = "fnord",
          deliveryConfigName = "fnord-manifest2",
          reference = "fnord-docker-stable",
          branch = "main"
        ),
        DockerArtifact(
    ...
    val c2 = ArtifactInEnvironmentContext(
      deliveryConfig = d2,
      environmentName = e2.name,
      // Same artifact version as other context
      artifactReference = c1.artifactReference,
      version = c1.version
    )
